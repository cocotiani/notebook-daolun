#define SSID        "Honor10" //改为你的Wi-Fi名称
#define PASSWORD    "18500804998dj"//Wi-Fi密码
#define HOST_NAME   "api.heclouds.com"
#define DEVICEID    "503130890" //OneNet上的设备ID
#define PROJECTID   "184025" //OneNet上的产品ID
#define HOST_PORT   (80)
String apiKey="He1s3lNMYWOeJYYYAmGk7IXEw=8=";//与你的设备绑定的APIKey
#include "OurSpark.h"
ColorLEDController LEDController1(2, 6);//定义灯引脚为d6
#define buzzer_pin 4    //定义蜂鸣器引脚d4
#define INTERVAL_SENSOR   17000             //定义传感器采样时间间隔  17000
...
{   
 ...
 do {
 setFont_L;
 u8g.setPrintPos(0, 10);//定义坐标
 u8g.print("light:");
 u8g.setPrintPos(50,10);
 u8g.print(sensor_lux);//光照
 ...
 if(sensor_lux<=700)//光照小于700
 {
    for(int i=0; i<256; ++i)  
  {
       LEDController1.ShowColor(0,i,0,0); //红色呼吸灯亮
       delay(10);
  }
    for(int i=255; i>=0; --i)
  {
    ...
    if(sensor_lux<=200)//光照小于200
    {
      tone(buzzer_pin,300);//蜂鸣器响,频率300
    } 
    else {
